...
public class DMRecipeProvider extends RecipeProvider {
    // 1.21.1: output is a parameter of buildRecipes, not a base-class field.
    // Hold it so the helper methods below can reach it.
    private RecipeOutput output;

    public DMRecipeProvider(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> registries) {
    ...
    protected void buildRecipes(RecipeOutput output) {
        ...
        ).unlockedBy("has_meat", has(DMItems.DRAGON_MEAT)).save(this.output, makeId("cooked_dragon_meat_form_" + desc)));
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.TOOLS, DMItems.VARIATION_ORB)
                ...
                .pattern("#U#").pattern("UOU").pattern("#U#")
...
//    }
    ...
    void dragonScaleAxe(Item scales, @Nullable Item result) {
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.TOOLS, result)
                .define('#', Tags.Items.RODS_WOODEN).define('X', scales)
    ...
    private void dragonScaleBlock(Item scales, @Nullable ItemLike result) {
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, scales, 9)
                ...
                .save(this.output, BuiltInRegistries.ITEM.getKey(scales));
        ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, result)
                .define('#', scales).pattern("###").pattern("###").pattern("###")
                .group("neodragonmounts.dragon_scale_block")
                .unlockedBy(getHasName(scales), has(scales))
                .save(this.output, BuiltInRegistries.ITEM.getKey(result.asItem()));
    }

    private void dragonScaleBow(Item scales, @Nullable Item result) {
        if (result == null) return;
        ShapedRecipeBuilder.shaped(RecipeCategory.COMBAT, result)
                .define('#', scales).define('X', Tags.Items.STRINGS)
    ...
    private void dragonScaleShield(Item scales, @Nullable Item result) {
        if (result == null) return;
        ShapedRecipeBuilder.shaped(RecipeCategory.COMBAT, result)
                .define('X', Tags.Items.INGOTS_IRON).define('W', scales)
                .pattern("WXW").pattern("WWW").pattern(" W ")
